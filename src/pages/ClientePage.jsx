...
const ClientePage = () => {
    ...
    // Lee ?mesa=N directamente desde la URL (síncrono) para el estado inicial.
    // Esto permite el bypass ANTES del primer render, sin esperar al useEffect de App.jsx.
    const [vista, setVista] = useState(() => {
        const mesaUrl = new URLSearchParams(window.location.search).get('mesa');
        if (mesaUrl) return 'menu';          // QR escaneado → directo al catálogo
        return numeroMesa ? 'menu' : 'ingreso';
    });

    const [ordenId, setOrdenId] = useState(ordenActual?.orden_id || null);

    // Si la mesa llega por URL, asegurarse de setearla en el store también
    useEffect(() => {
        const mesaUrl = new URLSearchParams(window.location.search).get('mesa');
        if (mesaUrl) {
            setNumeroMesa(parseInt(mesaUrl, 10));
        }
    }, [setNumeroMesa]);
    ...
    return (
        <div style={{ backgroundColor: '#FFDBBF', minHeight: '100vh', width: '100%' }}>
            {renderVista()}
        </div>
    );
};
