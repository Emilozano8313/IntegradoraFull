    ...
    return (
        <div className="d-flex flex-column min-vh-100" style={{ background: '#F4F5F7' }}>
            ...
            >
                ...
                {orden?.mesero_nombre && orden.mesero_nombre !== 'Auto-Pedido' && (
                    <div className="mt-2 d-inline-flex align-items-center bg-white bg-opacity-25 px-3 py-1 rounded-pill small shadow-sm">
                        🧑‍🍳 <span className="ms-2 fw-semibold">Atiende: {orden.mesero_nombre}</span>
                    </div>
                )}
            </header>
            ...
            <div className="container px-3" style={{ marginTop: '-2rem' }}>
                ...
                {/* ── Acciones finales ──────────────────────────────── */}
                {(estado === 'cerrada' || estado === 'entregada') && (
                    <button
                        className="btn btn-primary w-100 fw-bold py-3 mb-4"
                        style={{ borderRadius: '0.75rem' }}
                        onClick={onNuevoPedido}
                    >
                        {estado === 'entregada' ? 'Regresar al Menú (Pedir algo más)' : 'Hacer Nuevo Pedido'}
                    </button>
                )}
            </div>
        </div>
